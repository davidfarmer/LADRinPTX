...
\exercise
Suppose $T \in \L(U, V)$ and $S \in \L(V\1,W)$ are both invertible linear maps. Prove that $ST \in \L(U, W)$ is invertible and that $(ST)^{-1} = T^{-1} S^{-1}\!$.
...
\exercise
Suppose $V$ is a finite-dimensional vector space and $R, S, T \in
\L(V)$ are such that $RST$ is surjective.  Prove
that $S$ is  injective.
...
equations\index{inhomogeneous system of linear equations}
\begin{align*}
