...
\[
\det A = \sum_{(j_1, \ldots, j_n) \in \perm n} \paren[\bigl]{ \sign(j_1, \ldots, j_n) } A_{j_1,1} \cdots A_{j_n,n},
\]
If $(j_1, \ldots, j_n) \in \perm n$ and $j_1 \ne 1$, then $A_{j_1, 1} = 0$. Thus in the sum above, we can ignore all permuations except those permutations for which $j_1 = 1$. Thus
\begin{align*}
\det A &= A_{1,1} \sum_{(1, j_2, \ldots, j_n) \in \perm n} \paren[\bigl]{ \sign(1, j_2, \ldots, j_n) } A_{j_2,2} \cdots A_{j_n,n} \\[6bp]
&= A_{1,1} \sum_{(k_2, \ldots, k_n) \in \perm (n-1)} \paren[\bigl]{ \sign(k_2, \ldots, k_n) } A_{k_2+1,2} \cdots A_{k_n,n} \\[6bp]
&= A_{1,1} \det B.
\end{align*}
\end{Solution}
...
\abs{\det S} = \norm{S} = 1.
\]
Recall that $\norm{S}$ equals the largest singular value of $S$ (see \ref{7max}). Because the largest singular value of $S$ equals $1$, all the singular values of $S$ are less than or equal to $1$.

The equation $\abs{\det S} = 1$ and \ref{324} imply that the product of the singular values of $T$ equals $1$. Thus all the singular values of $T$ equal $1$. Now \ref{5isomsv1} shows that $S$ is an isometry and hence is a unitary operator.
\end{Solution}
...
\exercise
Suppose $V$ is a real vector space with even dimension, $T \in \L(V)$, and $\det T < 0$. Prove that $T$ has at least two distinct eigenvalues.
...
\exercise
Suppose $V$ is a real vector space with odd dimension and $T \in \L(V)$. Without using the minimal polynomial, prove that $T$ has an eigenvalue.\label{9oddeigen}
...
\begin{Solution}
The characteristic polynomial of $T$ has odd degree. Every polynomial with odd degree and real coefficients has a real zero, by Exercise \ref{4oddzero} in Chapter \ref{173}. Thus $T$ has an eigenvalue by \ref{9eigdet}.
\end{Solution}
...
p(z) = \det(zS - T).
\]
Prove that $p$ is a polynomial with degree $\dim V$ and that the coefficient of $z^{\dim V}$ in this polynomial is $\det S$.
...
\begin{Solution}
Suppose that $\la_1, \ldots, \la_n$ are the eigenvalues of $T$, each included as many times as its multiplicity. If $x \in \F{}$, then the eigenvalues of $I + x T$ are
\[
...
&= f' \! (t)
\end{align*}
for some $t \in (0, 1)$ [by the mean value theorem]. The multi-variable version of the Chain Rule implies that $f$ is indeed differentiable on the interval $(0, 1)$ and
\[
...
\begin{Solution}
Let $E$ denote the ellipsoid defined above and let $\Omega$ be the open ball with radius
$1$ defined by
\[
...
\begin{Solution}
Let $A = QR$ be the unique $QR$ decomposition of $A$, where $Q$ is unitary and $R$ is upper triangular with only positive numbers on its diagonal.
...
$j > k$). The proof of the $QR$ decomposition (\ref{7QR}) shows that this happens if and only if $v_k$ is orthogonal to $e_j$ for each
...
\]
for all $A, B \in \C{n,n}$ and $\delta(A)$ equals the product of the diagonal entries of $A$ for each diagonal matrix $A \in \C{n,n}$ (recall that $\C{n,n}$ denotes the vector space of \by{n}{n} matrices with entries in $\C{}$). Prove that
$\delta(A) = \det A$ for all $A \in \C{n,n}$.
\exercisecomment{This exercise shows that the determinant is the unique function defined on square matrices that is multiplicative and has the desired behavior on diagonal matrices. This result is analogous to Exercise \ref{9trChar} in Section \ref{7trace}, which shows that the trace is uniquely determined by its algebraic properties.}
