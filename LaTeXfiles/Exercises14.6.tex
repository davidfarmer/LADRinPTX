...
\[
\norm{T} = \max \br{ \abs{\la} : \la \text{ is an eigenvalue of } T}.
\]
\exercisedisplayend
...
\exercise
Suppose $U$ is an inner product space, $T \in \L(V\1, U)$ and $S \in \L(U, W)$. Prove that \label{2prod}
\[
\norm{ST} \le \norm{S} \, \norm{T}.
\]
\exercisedisplayend
...
\exercise
Suppose $S, T \in \L(V)$ are positive. Show that \mbox{$\norm{S+T} \ge \max\br{ \norm{S}, \norm{T} }$}.

\begin{Solution}
Let $\la$ denote the largest eigenvalue of $S$ and let $e \in V$ be a corresponding eigenvector with $\norm{e} = 1$. The third bullet point of \ref{5exnorms} implies that $\norm{S} = \la$.
...
$\norm{\phi_u} = \norm{u}$.
\exercisecomment{Here we are thinking of the scalar field\/ $\F{}$ as an inner product space with\/ $\ip{\al,\beta} = \al \overline{\beta}$ for all\/ $\al, \beta \in \F{}$. Thus\/ $\norm{\phi_u}$ means the norm of\/ $\phi_u$ as a linear map from\/ $V$ to\/ $\F{}$.}

\begin{Solution}
Thinking of $\F{}$ as an inner product space, we have $\norm{\al} = \abs{\al}$ for all $\al \in \F{}$.
...
$\paren[\big]{{s_1}^{\2} + \cdots + {s_n}^{\2}}^{1/2}$, as was shown in Exercise \ref{4sums2} in Section \ref{2SVD}.}
...
Let $s_1, \ldots, s_n$ denote the singular values of $T\3$. Exercise \ref{4sums2} in Section \ref{2SVD} shows that
\[
...
Thus
\[
\norm{Te_1}^2 + \cdots + \norm{Te_n}^2 = \norm{T}^2 +{s_2}^{\2} + \cdots + {s_n}^{\2}.
\]
Thus
\[
\norm{Te_1}^2 + \cdots + \norm{Te_n}^2 = \norm{T}^2
\]
if and only if $s_2 = \cdots = s_n = 0$, which happens if and only if $\dim \ran T \le 1$.
\end{subtheorem}
\end{Solution}

\exercise
Prove that if $T \in \L(V\1, W)$, then  $\norm[\big]{T^*T} = \norm{T}^2$.\index{C*@$C^*$-algebras}\label{8Cstar}
...
The equation above implies that $\norm[\big]{T^2} = \norm{T}^2$. Applying this result to the normal operator $T^2$ shows that
$\norm[\big]{T^4} = \norm[\big]{T^2}^2 = \norm{T}^4$. Iterating this procedure, we see that
\[
\norm[\Big]{ T^{2^n} } = \norm{T}^{2^n}
\]
...
$\dim U^\perp = n-k$ and hence (see Exercise \ref{5dimST} in Section \ref{nullspace})
\[ \tag{$(*)$}
...
&\le \norm{v}^2 \max \br{ \abs{s_1 -1}, \ldots, \abs{s_n - 1} }^2,
\end{align*}
which shows that
\[
...
\uppar{as in the polar decomposition \ref{244}}, then the positive operator equals\/
$\sqrt{T^*T}$.}
...
\]
equals the ball in $\ran T$ centered at $0$ with radius $1$.

\begin{Solution}
Let $B$ denote the ball in $\ran T$ centered at $0$ with radius $1$.
