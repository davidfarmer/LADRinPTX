...
\exercise
Suppose $T \in \L(V\1, W)$. Show that $T = 0$ if and only if all the singular values of $T$ equal $0$.

\begin{Solution}
If $T = 0$, then $T^*T=0$ and hence all the singular values of $T$ equal~$0$.

To prove the implication in the other direction, suppose all the singular values of $T$ equal $0$. Then \ref{7posSV}(b) shows that
...
\[
\norm{Tv}^2 = {s_1}^{\2} {c_1}^{\2} + \cdots + {s_m}^{\2} {c_m}^{\2}.
\]
This shows that the values of $\norm{Tv}^2$ that we can get from this process are weighted averages of ${s_1}^{\2}, \ldots, {s_m}^{\2}$. Hence this process produces the interval $[s_m, s_1]$ for the values of $\norm{Tv}$ as $v$ ranges over the subset of $V$ with
...
\norm{Tv}^2 = {s_1}^{\2} {c_1}^{\2} + \cdots + {s_m}^{\2} {c_m}^{\2}.
\]
This shows that the values of $\norm{Tv}^2$ that we can get from this process are weighted averages of ${s_1}^{\2}, \ldots, {s_m}^{\2}, 0$. Hence this process produces the interval $[0, s_1]$ for the values of $\norm{Tv}$ as $v$ ranges over the subset of $V$ with
$\norm{v} = 1$.
\end{Solution}
...
$\abs{\la_1}, \ldots, \abs{\la_n}$, after these numbers are sorted into decreasing order.\label{7singsp}
...
\item
Prove that if $k \in \br{1, \ldots, m}$, then $e_k$ is an eigenvector of $T^*T$ with corresponding eigenvalue ${s_k}^{\2}$.
\item
Prove that if $k \in \br{1, \ldots, m}$, then $f_k$ is an eigenvector of $TT^*$ with corresponding eigenvalue ${s_k}^{\2}$.
\end{subtheorem}
...
\]
for every $v \in V\1$. Thus the positive eigenvalues of $T^* T$ are ${s_1}^{\2}, \ldots, {s_k}^{\2}$. Hence the singular values of $T$ are
$s_1, \ldots, s_m$.
...
\]
for every $v \in V\1$. Hence if $k \in \br{1, \ldots, m}$, then $T^*Te_k = {s_k}^{\2} e_k$; thus $e_k$ is an eigenvector of $T^*T$ with corresponding eigenvalue ${s_k}^{\2}$.
...
\]
for every $w \in W\3$. Hence if $k \in \br{1, \ldots, m}$, then $TT^*f_k = {s_k}^{\2} f_k$; thus $f_k$ is an eigenvector of $TT^*$ with corresponding eigenvalue ${s_k}^{\2}$.
\end{subtheorem}
\end{Solution}
...
The equation above implies that the positive eigenvalues of $TT^*\1$, each included as many times as the corresponding eigenspace, are ${s_1}^{\2}, \ldots, {s_m}^{\2}$. Thus the singular values of $T^*$ are $s_1, \ldots, s_m$, as desired.
\end{Solution}
...
\exercise
Suppose that $T \in \L(V\1, W)$ and $v_1, \ldots, v_n$ is an orthonormal basis of $V\1$. Prove that \label{4sums2}
\[
\norm{Tv_1}^2 + \cdots + \norm{Tv_n}^2 = {s_1}^{\2} + \cdots + {s_n}^{\2},
\]
where $s_1, \ldots, s_n$ are the singular values of $T\3$.
\exercisecomment{See the comment after Exercise \ref{6sumT2} in Section \ref{7001}.}

\begin{Solution}
Exercise \ref{6sumT2} in Section \ref{7001} shows that the left side of the equation above is independent of the choice of orthonormal basis (and clearly, so is the right side). Thus we only need to prove that the equation above holds for at least one orthonormal basis of $V\1$.
...
\]
where the first equality holds because $T^*T = TT^*\1$. The equation above shows that $v \in E\paren[\big]{\la^2, (T^2)^* T^2}$. Hence
\[
E(\la, T^* T) \subset E\paren[\big]{\la^2, (T^2)^* T^2},
\]
which implies that
\[
\dim E(\la, T^* T) \le \dim E\paren[\big]{\la^2, (T^2)^* T^2}
\]
...
\exercisecomment{Compare the result in this exercise to the analogous result for invertible linear maps \textup{[}see \ref{7propadjoint}\uppar{f}\textup{]}.}
