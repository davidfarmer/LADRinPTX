...
\begin{Solution}
First suppose $a_1, \ldots, a_n \in \F n$. Then
\begin{align*}
...
\exercise
Suppose $V$ and $W$ are finite-dimensional and $T \in \L(V\1, W)$. Prove that $T$ is invertible if and only if $T' \in \L\paren[\big]{W', V'}$ is invertible.
...
\begin{Solution}
If $u \in U$, then $\phi(u) = 0$ for every $\phi \in U^0\!$. Thus
\[
...
\begin{subtheorem}
\item
First suppose $U \subset W\3$. Suppose $\phi \in W^0\!$. If $u \in U$, then $u \in W\3$, and hence $\phi(u) = 0$. Thus $\phi \in U^0\!$. Hence $W^0 \subset U^0\!$.

To prove the implication in the other direction, now suppose $W^0 \subset U^0\!$. Suppose $u \in U$. Thus $\phi(u) = 0$ for every $\phi \in U^0\!$. Hence $\phi(u) = 0$ for every $\phi \in W^0\!$. Now Exercise \ref{3Uan} implies that $u \in W\3$. Thus $U \subset W\3$, as desired.

\item
If $U = W\3$, then clearly $W^0 = U^0\!$.

To prove the implication in the other direction, now suppose $W^0 = U^0\!$. Then $W^0 \subset U^0$ and $U^0 \subset W^0\!$. Now (a) implies that $U \subset W$ and $W \subset U$. Thus $U = W\3$, as desired.
\end{subtheorem}
\end{Solution}
...
\begin{subtheorem}
\item
First suppose $\phi \in (U + W)^0\!$. Then $\phi(u + w) = 0$ for all $u \in U$ and all $w \in W\3$. Taking $w = 0$ and then taking $u = 0$, in particular we see that $\phi(u) = 0$ for all $u \in U$ and $\phi(w) = 0$ for all $w \in W\3$. Thus $\phi \in U^0$ and $\phi \in W^0\!$. In other words, $\phi \in U^0 \cap W^0\!$. Thus $(U + W)^0 \subset U^0 \cap W^0\!$.

To prove the inclusion in the other direction, suppose $\phi \in U^0 \cap W^0\!$. If $u \in U$ and $w \in W\3$, then
\[
\phi(u + w) = \phi(u) + \phi(w) = 0 + 0 = 0.
\]
Hence $\phi \in (U+W)^0\!$. Thus $(U + W)^0 \supset U^0 \cap W^0\!$.

Thus $(U + W)^0 = U^0 \cap W^0$, as desired.

\item
First suppose $\phi \in U^0 + W^0\!$. Thus $\phi = \phi_1 + \phi_2$, where $\phi_1 \in U^0$ and $\phi_2 \in W^0\!$. If $v \in U \cap W\3$, then
\[
\phi(v) = \phi_1(v) + \phi_2(v) = 0 + 0 = 0.
\]
Hence $\phi \in (U \cap W)^0\!$. Thus
\[
U^0 + W^0 \subset (U \cap W)^0\!.
\]
...
$\psi \in (\ker \phi_1)^0\!$. Thus $\ker \phi_1 \subset \ker \psi$. Now Exercise \ref{7nullsub} implies $\psi \in \Span(\phi_1)$, as desired.
...
If $(c_1, \ldots, c_m) \in \F m$, then by $(*)$ there exists $u \in V$ such that $\phi_k(u) = c_k$ for each $k = 1, \ldots, m-1$. Thus
\[
...
\[
\Omega = \br[\big]{v \in V: \phi(v) = 0 \text{\ for every } \phi \in \Omega}^0\!.
\]
\exercisedisplayend
...
&= \{v \in V: \phi(v) = 0 \text{ for every } \phi \in \Omega\}^0,
 \end{align*}
...
First consider the case $\psi = 0$. In this case, $\ran T' = \{0\}$. Thus $\dim \ran T' = 0$, which implies that $\dim \ran T = 0$ [by \ref{rangeTprime}(a)]. Thus $T = 0$, which means that $\ker T = V\1$. Therefore $\ker T = \ker \psi$ because both sides of this equation equal $V$ in this case where $\psi = 0$.
...
\item
Show that $\ran \pi' = U^0\!$.
\item
Conclude that $\pi'$ is an isomorphism from $(V\1/U)'$ onto $U^0\!$.
\end{subtheorem}
...
\]
and thus $\pi' \! (\phi) \in U^0\!$. Hence $\ran \pi' \subset U^0\!$.

To show the inclusion in the other direction, suppose $\psi \in U^0\!$. Thus $\psi \in V'$ and $\psi(u) = 0$ for all $u \in U$. Define $\phi \in (V\1/U)'$ by
\[
\phi(v+ U) = \psi(v);
\]
the condition that $\psi(u) = 0$ for all $u \in U$ shows that $\phi$ is well defined. The definitions now show that $\pi' \! (\phi) = \psi$. Thus $\ran \pi' \supset U^0$, completing the proof that $\ran \pi' = U^0\!$.

\item
Now (a) and (b) immediately imply that $\pi'$ is an isomorphism from $(V\1/U)'$ onto $U^0\!$.
\end{subtheorem}
\end{Solution}
