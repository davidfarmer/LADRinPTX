...
\exercise
Suppose $\la \in \F{}$ and $m$ is a positive integer.
\begin{subtheorem}
\item
Explain why $1, z-\la, \ldots, (z-\la)^m$ is a basis of $\P\1_m(\F{})$.
\item
Suppose $p \in \P(\F{})$ and $p(\la) = 0$. Use (a) to prove that there exists $q \in \P(\F{})$ such that $p(z) = (z-\la) q(z)$ for all
$z \in \F{}\3$.
\end{subtheorem}
\exercisecomment{The intent of \uppar{b} is to provide an alternative proof of \ref{460} that uses linear algebra instead of the division algorithm.}
...
polynomials with coefficients in $\F{}$ and of degree $m$ is not a
subspace of $\P(\F{})$ because it is not closed under addition.  Specifically,
the sum of two polynomials of degree $m$ may be a polynomial of degree less
...
\begin{Solution}
The set consisting of $0$ and all polynomials of even degree is not a subspace of $\P(\F{})$ because it is not closed under addition.  Specifically, the sum of two polynomials of even degree may be a polynomial of odd degree.  For example, $7 + 4z + 5z^2$ and
$1 + 2z - 5z^2$ are both polynomials of degree $2$ but their sum, which equals $8 + 6z$, is a polynomial of odd degree.
\end{Solution}
...
\]
Then $p$ is a polynomial of degree $n$ and exactly the required zeros.
\end{Solution}
...
zeros, which means that $p = 0$ (by~\ref{4polyzero}).  Thus $T$ is injective, as
desired.
...
&= \dim \FF{m+1},
\end{align*}
where the first equality comes from the fundamental theorem of linear maps (\ref{3ab11}) and the second equality holds
...
\begin{Solution}
Suppose $p$ is a polynomial of odd degree and real coefficients.
...
polynomial of odd degree $m$.  We can write $p$ in the form
\[
...
\begin{Solution}
Suppose $p \in \P(\R{})$. Clearly $3$ is a zero of the polynomial $p - p(3)$. Thus by \ref{460}, there exists a polynomial $q \in \P(\R{})$ such that
\[
p(x) - p(3) = (x - 3) q(x)
\]
...
\]
for every $x \in \R{}$. Now \ref{4polyzero} implies that $\im b_0 = \im b_1 = \dots = \im b_{2n} = 0$. Thus $q$ has real coefficients.
\end{Solution}
...
\]
for every $x \in \R{}$. Our hypothesis states that there are at least $m+1$ distinct real zeros of the polynomial $\im p$. Thus \ref{4polyzero} and the equation above imply that
\[
\im a_0 = \im a_1 = \dots = \im a_m = 0.
\]
Thus all coefficients of $p$ are real.
\end{Solution}
...
p(z) = a_0 + a_1 z + \dots + a_n z^n
\]
for every $z \in \F{}\3$. Thus $\deg p = n$.
...
\]
However, every element of $U$ is a polynomial of degree bigger than or equal to $n$ or is the $0$ polynomial. Thus $b_0 + b_1 z + \dots + b_{n-1} z^{n-1}$ is the zero polynomial, which implies that
...
\]
is a polynomial of degree less than $m$. Thus by our induction hypothesis
\[
