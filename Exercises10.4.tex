...
\begin{Solution}
Let $v_1, v_2$ be a linearly independent list in $V$ and let $w_1, w_2$ be a linearly independent list in $W\3$. Suppose there exist $v \in V$ and $w \in W$ such that
\[
...
\]
The matrix above has rank at most one because every row is a scalar multiple of $(w_1, \ldots, w_n)$, or we can see that the matrix has rank at most one because every column is a scalar multiple of $\tran{(v_1, \ldots, v_m)}\3$.
...
\begin{Solution}
Using the standard bases for $\F m$ and $\F n$, we identify $\F m \tensor \F n$ with the vector spaces of \by{m}{n} matrices, as in Example \ref{9FmFn}. Under this identification, $\br{ v \tensor w : v \in \F m \text{ and } w \in \F n }$ is identified with the set of matrices of rank at most one, as discussed in Exercise \ref{9rank1}.

A linear map from $\F n$ to $\F m$ has range of dimension at most two if and only if it is the sum of  two linear maps each of which has range of dimension at most one. Thus the set of \by{m}{n} matrices of rank at most two can be identified with the set
\[
...
\br{ v_1 \tensor w_1 + v_2 \tensor w_2 : v_1, v_2 \in V \text{ and } w_1, w_2 \in W }
\]
gets identified with the set of \by{m}{n} matrices (with entries in $\F{}$) that have rank at most two. Because $m > 2$ and $n > 2$, there exist \by{m}{n} matrices of rank greater than two. Hence
\[
\br{ v_1 \tensor w_1 + v_2 \tensor w_2 : v_1, v_2 \in V \text{ and } w_1, w_2 \in W } \ne V\1 \tensor W\3.
\]
\end{Solution}
...
\begin{Solution}
By \ref{9bi2linear}(a), there exists a linear map $\fun {\hat[8]{\Gamma}} {V\1 \tensor W} U$ such that
\[
...
Now
\begin{align*}
\Gamma(v_1, w_1) + \cdots + \Gamma(v_m, w_m) &= \hat[8]{\Gamma}(v_1 \tensor w_1) + \cdots + \hat[8]{\Gamma}(v_w \tensor w_m) \\
&= \hat[8]{\Gamma}(v_1 \tensor w_1 + \cdots + v_m \tensor w_m ) \\
&= \hat[8]{\Gamma}(0) \\
&= 0,
\end{align*}
as desired.
...
&= Sv \tensor Tw
\end{align*}
as desired, where the second line holds because $S \tensor T$ is linear, the third line holds by the definition of $S \tensor T$, and the fourth line holds because the tensor product is bilinear.
...
\begin{Solution}
Suppose $e_1^{\,k}, \ldots, e_{n_k}^{\,k}$ is an orthonormal basis of $V\1_k$ for $k = 1, \ldots, m$.
