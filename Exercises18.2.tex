...
\begin{Solution}
Let $n = \dim V$. Let $W$ be the vector space of functions from $\br{1, \ldots, n}^m$ to $\F{}$, with the usual operations of addition and scalar multiplication of functions. Then $W$ is isomorphic to $\F {(n^m)}$. Hence $\dim W = n^m\!$.
...
\]
and extend $\beta_A$ using multilinearity to an element of $V^{(m)}$ (see the definition of $\beta_A$ in the proof of \ref{9dimV2}, which is the case when $m=2$). Then $\Gamma(\beta_A) = A$, showing that $\Gamma$ is surjective.

Bcause $\Gamma$ is an isomorphism from $V^{(m)}$ to $W$, we see that
\[
\dim V^{(m)} = n^m\!.
\]
\end{Solution}
...
Now suppose $v_1, \ldots, v_m$ is a list of vectors in $V$ with $v_j = v_k$ for some $j, k \in \br{1, \ldots, m}$ with $j \ne k$. Swap consecutive slots to bring these two vectors adjacent to each other. This process changes the value of $\al$ by a factor of $(-1)^N$, where $N$ is the total number of swaps needed to bring the two equal vectors adjacent to each other. Once these vectors are adjacent, our hypothesis states that the value of $\al$ is $0$.
\end{Solution}

\exercise
Prove or give a counterexample: If $\al \in V^{(4)}_\text{alt}$, then
\[
\br{ (v_1, v_2, v_3, v_4) \in V^4 : \al(v_1, v_2, v_3, v_4) = 0 }
\]
...
\]
Thus $\br{ (v_1, v_2, v_3, 4) \in V^4 : \al(v_1, v_2, v_3, v_4) = 0 }$ is not closed under addition and hence is not a subspace of $V^4\!$.
\end{Solution}
...
\[
\al(v_1, \ldots, v_m) =
\sum_{(j_1, \ldots, j_m) \in \perm m} \paren[\bigl]{ \sign(j_1, \ldots, j_m) } \beta( v_{j_1}, \ldots, v_{j_m} )
\]
for $v_1, \ldots, v_m \in V\1$. Explain why $\al \in V^{(m)}_{\text{alt}}$.
